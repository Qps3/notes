...
实现`List`接口并非只能通过数组（即`ArrayList`的实现方式）来实现
...
使用`List`来实现存在效率非常低的问题，因为平均需要扫描一半的元素才能确定，
...
如果`key`的值为`"a"`，计算得到的索引总是`1`，因此返回`value`为`Person("Xiao Ming")`，
...
但是我们自己无法按定义普通`class`那样来定义`enum`，必须使用`enum`关键字，这是Java语法规定的。
...
我们通常可以用一个内部类来实现`Iterator`接口，

这个内部类可以直接访问对应的外部类的所有字段和方法。



例如，上述代码中，内部类`ReverseIterator`可以用`ReverseList.this`获得当前外部类的`this`引用，

然后，通过这个`this`引用就可以访问`ReverseList`的所有字段和方法。
...
![image-20220608111042293](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220608111042293.png)

![image-20220609172615190](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609172615190.png)

 

开发时根据不同的需求选择不同的集合 

list集合(有序，并且能存储重复值的集合)
...
此外，也可以用各个集合接口提供的`of(T...)`方法创建空集合。

例如，以下创建空`List`的两个方法是等价的：
...
>
> > 对自定义类进行排序，list是要排序的对象，然后comparator是进行排序的工具 



## 静态方法：

使用static 之后使用	类名称.该方法	即可调用	(可以更好的调用)
...
Collection<T>  c		 第一个形参表示的向哪一个集合添加元素(即collection的一个集合，包括list 与 set下的实现类)

T...element				 第二个形参表示的是添加的是哪些元素，不知道要添加多少个，所以是可变参数的写法 



方法种的<T>会根据后面collection<T>中的<T>确定类型，

后面可变参数的<T>也会随之发生变化，T...element是可变参数的写法



![image-20220609121402315](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609121402315.png)
...
```java
public  static  void  shuffle(List<?>);
	//用于将List集合中元素顺序打乱(例如用于实现斗地主的功能)
	//<?>泛型通配符 
```



list（有序集合,可以进行顺序打乱操作）

set（无索引，不重复，无序）集合

![image-20220609121306178](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609121306178.png)
...
> 类里面都重写了Comparable接口里的ComparableTo方法
...
如果是字母String会根据abcdef排序

![image-20220609155248506](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609155248506.png)
...
![ ](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609122154466.png)
...
因为String 与 Integer 都是类，他们的类里面都重写了Comparable接口里的ComparableTo方法，然后才可以被Collection.sort( );调用
...
我们应该自定义排序规则，sort()方法会主动调用类里面的排序规则、



==类需要实现—个Comparable接口，重写它里面的ComparableTo的方法==



**没有实现comparable接口的compareTo时，会报错**

![image-20220609123300117](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609123300117.png)
...
![image-20220609123824902](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609123824902.png)
...
![image-20220609124048754](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609124048754.png)

2. 由大到小的排序方式

![image-20220609124205235](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609124205235.png)



##### **截图总结**

![image-20220609161245940](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609161245940.png)
...
> List<T> list                   list的一个对象
>
> Comparator<T> c      comparator接口下的一个实现类new 的一个对象
...
![image-20220609125537371](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609125537371.png)



下面是继承comparator 的实现类，该类被实例一个对象，用于

```apl
Collections.sort (list,实例对象);
```

 中，collections.sort根据实例对象里面的规则进行排序

![image-20220609125353370](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609125353370.png)
...
如下图，直接在方法调用中加入匿名内部类进行实现

![image-20220609162316628](C:\Users\16660\AppData\Roaming\Typora\typora-user-images\image-20220609162316628.png)
