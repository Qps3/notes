...
iBatis一词来源于“internet”和“abati



s”的组合，是一个基于Java的持久层框架。 iBatis提供的持久层框架包括SQL Maps和Data Access Objects（DAO）。
...
- JDBC
  - SQL 夹杂在Java代码中耦合度高，导致硬编码(写死在java中)内伤
    - **java工程一般打包成jar包**
      - 编译、执行、打包
    - **web工程一般打包成war包**
    - 所以如果需要修改，需要再 编译、执行、打包
...
2. 打开项目结构
   - 选择jdk版本
   - 选择对应语言选项
   - <img src="assets/202304141931428.png" style="float: left; zoom: 50%;" />

3. 打开设置，设置MAVEN
   - <img src="assets/202304141934809.png" alt="image-20230414193416727" style="float:left;zoom:50%;" />

4. 创建一个maven module项目

   > 用到的链接
   >
   > > [idea中使用Maven](https://blog.csdn.net/hgnuxc_1993/article/details/125427590)
   > >
   > > [IDEA根据maven模板创建工程](https://zhuanlan.zhihu.com/p/533247406)

   1. <img src="assets/202304141936115.png" alt="image-20230414193641062" style="float:left;zoom:80%;" />

   2. <img src="assets/202304141958858.png" alt="image-20230414195808792" style="float:left;zoom: 50%;" />

   3. <img src="assets/202304141958506.png" alt="image-20230414195853430" style="float:left;zoom: 50%;" />

   4. 修改配置文件，新增打包为jar包的方式

      > <packaging>jar</packaging>
      >
      > > <img src="assets/202304142002392.png" alt="image-20230414200218317" style="float:left;zoom:80%;" />

      > **目前并不需要打包war包，因为目前mybatis只需要java**
      >
      > 

   5. 在maven中加入下面的配置依赖的文件
   ...
   6. 认识结构
      ...
      > > ![image-20230414201331869](assets/202304142013942.png)
...
<!DOCTYPE configuration
        PUBLIC "-//mybatis.org//DTD Config 3.0//EN"  <!-- 指定了用于验证这个XML文档的文档类型定义 (DTD) -->
        "http://mybatis.org/dtd/mybatis-3-config.dtd">
<configuration>  <!-- MyBatis 配置的根元素 -->
    
	<!--设置连接数据库的环境-->
	<environments default="development">  <!-- 定义数据库环境，设置默认环境为development -->
		<environment id="development">  <!-- 具体定义development环境的数据库连接信息 -->
			<transactionManager type="JDBC"/>  <!-- 使用JDBC方式进行事务管理 -->
			<dataSource type="POOLED">  <!-- 使用连接池的数据源 -->
				<property name="driver" value="com.mysql.cj.jdbc.Driver"/>  <!-- MySQL数据库的JDBC驱动 -->
				<property name="url" value="jdbc:mysql://localhost:3306/ssm?serverTimezone=UTC"/> 
                <!-- MySQL数据库的URL -->
				<property name="username" value="root"/>  <!-- 连接MySQL数据库的用户名 -->
				<property name="password" value="123456"/>  <!-- 连接MySQL数据库的密码 -->
			</dataSource>
		</environment>
	</environments>
	<!--引入映射文件-->
	<mappers>  <!-- 定义映射器 -->
		<package name="mappers/UserMapper.xml"/>  <!-- 指定UserMapper.xml文件，包含了SQL语句和结果映射的定义 -->
	</mappers>
</configuration>
